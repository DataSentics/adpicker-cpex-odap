...
df_silver_cpex_piano = read_silver(
    df_bronze_cpex_piano,
    dict_process_dates["start_date"],
    dict_process_dates["end_date"],
)
...
def aggregate_traits(df: DataFrame, n_days, end_date):
    return (
        df.withColumn("TRAIT", F.explode("userParameters"))
        .groupBy("USER_ID", "TRAIT")
        .agg(F.min(F.col("DATE")).alias("FIRST_DATE"))
...
df_aggregated_traits = aggregate_traits(
    df_silver_cpex_piano, widget_n_days, dict_process_dates["end_date"]
)
...
def aggregate_segments(df: DataFrame, n_days, end_date):
    return (
        df.withColumn("SEGMENT", F.explode("userParameters"))
...
df_aggregated_segments = aggregate_segments(
    df_silver_cpex_piano, widget_n_days, dict_process_dates["end_date"]
)
