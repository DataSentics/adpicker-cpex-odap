...
col_to_concat = ["URL_NORMALIZED_KEYWORDS", 
                 "URL_TITLE", 
                 "URL_KEYWORDS",
                 "URL_DESCRIPTIONS"
                 ]
...
def create_url_table():    
    if not delta_table_exists(get_value_from_yaml("paths", "sdm_table_paths", "sdm_url")):
...
df_silver_sdm_url = spark.read.format("delta").load(get_value_from_yaml("paths", "sdm_table_paths", "sdm_url"))
...
def read_cleansed_data(df_silver: DataFrame, df_url: DataFrame, logger: Logger):
    ...
    destination_max_date = get_max_date(df=df_url, date_col='DATE_ADDED', datetime_col='TIME_ADDED', last_n_days=20)
    ...
    if destination_max_date is not None:
        ...
        df_silver = (df_silver
                     .filter(F.col("day") >= destination_max_date_safe)
                     .filter(F.col("EVENT_TIME") > destination_max_date)
                    )
        
    return (df_silver
            .filter(F.col("rp_pageurl").isNotNull())
            .select(
                "rp_pageurl",
                "rp_pagetitle",
                "rp_pagekeywords",
...
df_cleansed_data = read_cleansed_data(df_bronze_cpex_piano, df_silver_sdm_url, root_logger)
...
def normalize_url_data(df: DataFrame):
    return df_url_normalization(df.withColumn("URL_NORMALIZED", F.col("rp_pageurl")), "URL_NORMALIZED")
...
def get_unique_url(df_cleansed: DataFrame, df_url: DataFrame):
    return (df_cleansed
            .join(df_url, how='left_anti', on='URL_NORMALIZED')
            ...
            .agg(
                ...
                F.first('rp_c_p_publishedDateTime', ignorenulls=True).alias('rp_c_p_publishedDateTime'),
...
def check_for_duplicates(df, logger: Logger):
    ...
    num_unique = df.groupby("URL_NORMALIZED").count().select(F.max("count")).collect()[0][0]
    if num_unique == 1:
...
def extract_url_formats(df: DataFrame):
    ...
    df = df_url_to_domain(df.withColumn("URL_DOMAIN_FULL", F.col("rp_pageurl")), "URL_DOMAIN_FULL", subdomains = 0)
    df = df_url_to_domain(df.withColumn("URL_DOMAIN_1_LEVEL", F.col("rp_pageurl")), "URL_DOMAIN_1_LEVEL", subdomains = 1)
    df = df_url_to_domain(df.withColumn("URL_DOMAIN_2_LEVEL", F.col("rp_pageurl")), "URL_DOMAIN_2_LEVEL", subdomains = 2)
    # ADFROM FORMAT
    return df_url_to_adform_format(df.withColumn("URL_ADFORM_FORMAT", F.col("rp_pageurl")), "URL_ADFORM_FORMAT")
...
def extract_page_info(df: DataFrame):
    return (df
            .withColumnRenamed("rp_c_p_pageclass","PAGE_TYPE")
...
def get_url_tokens_all(df_augmented: DataFrame):
    ...
    return (df_augmented
            .withColumn("URL_TOKENS_ALL_CLEANED", F.concat(*col_to_concat_cleaned))
            .withColumn("URL_TOKENS_ALL_STEMMED", F.concat(*col_to_concat_stemmed))
            .withColumn("URL_TOKENS_ALL_CLEANED_UNIQUE", F.array_distinct("URL_TOKENS_ALL_CLEANED"))
            .withColumn("URL_TOKENS_ALL_STEMMED_UNIQUE", F.array_distinct("URL_TOKENS_ALL_STEMMED"))
            .drop("rp_pageurl")
           )
...
def get_bigrams(df: DataFrame, suffix="BIGRAMS"):
    ...
    return (df
            .withColumn(f"URL_TOKENS_ALL_CLEANED_{suffix}", F.concat(*col_to_concat_cleaned_ngrams))
            .withColumn(f"URL_TOKENS_ALL_STEMMED_{suffix}", F.concat(*col_to_concat_stemmed_ngrams))
            .withColumn(f"URL_TOKENS_ALL_CLEANED_UNIQUE_{suffix}", F.array_distinct(f"URL_TOKENS_ALL_CLEANED_{suffix}"))
            .withColumn(f"URL_TOKENS_ALL_STEMMED_UNIQUE_{suffix}", F.array_distinct(f"URL_TOKENS_ALL_STEMMED_{suffix}"))
           )
...
write_dataframe_to_table(df_save_url_table, 
                         get_value_from_yaml("paths", "sdm_table_paths", "sdm_url"), 
                         schema_sdm_url, 
                         "append", 
