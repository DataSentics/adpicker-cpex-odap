...
from schemas import get_schema_sdm_pageview
...
df_silver_sdm_preprocessed = spark.read.format("delta").load(get_value_from_yaml("paths", "sdm_table_paths", "sdm_preprocessed"))

# COMMAND ----------

def add_pageview_ids_and_empty_cols(df: DataFrame):
    return (df
            .withColumn("PAGE_VIEWS", F.explode("PAGES_TIMES"))
            .withColumn("URL", F.col("page_views").getItem(0)).filter(~(F.col("URL") == ""))
...
df_added_pageview_ids_and_empty_cols = add_pageview_ids_and_empty_cols(df_silver_sdm_preprocessed)
...
# df_pageview_table.printSchema()

# write_dataframe_to_table(df_pageview_table,
