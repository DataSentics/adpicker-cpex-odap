...
from date_functions import get_max_date
from sdm_table_functions import sdm_hash
from yaml_functions import get_value_from_yaml

from schemas import get_schema_sdm_preprocessed, get_schema_sdm_session, get_schema_sdm_pageview
from _functions_nlp import df_url_normalization


# COMMAND ----------

tablet_names = ["ipad", "lnv_tab", "galaxy_tab"]
device_brand_names = ['apple', 'nokia', 'huawei', 'samsung', 'xiaomi', 'aligator', 'motorola', 'lenovo', 'lg', 'google']
device_marketing_names = ['iphone', 'mac', 'ipad', 'lnv_tab', 'galaxy_tab', 'galaxy', 'velvet', 'smart_tv', 'P30_lite', 'redmi', 'mi', 'pixel']
...
if not delta_table_exists(get_value_from_yaml("paths", "sdm_table_paths", "sdm_session")):
    
    schema, info = get_schema_sdm_session()
    df_empty = spark.createDataFrame([], schema)
    
    write_dataframe_to_table(df_empty, 
...
if not delta_table_exists(get_value_from_yaml("paths", "sdm_table_paths", "sdm_pageview")):
...
def read_cleansed_data(df: DataFrame, df_pageview: DataFrame, logger: Logger):
    ...
    destination_max_date = get_max_date(df=df_pageview, date_col='page_screen_view_date', datetime_col='page_screen_view_timestamp', last_n_days=20)
    logger.info(f"maximal date in silver.sdm_session before append: {destination_max_date}")
    
    if destination_max_date is not None:
        df = (df
              .filter(F.col("day") >= F.to_date(F.lit(destination_max_date.date())))
    ...
    df = df.withColumn("SEARCH_ENGINE", F.coalesce(df.referrer_search_engine,df.pageurl_search_engine)) 
...
df_cleansed_data = read_cleansed_data(df_bronze_cpex_piano, df_silver_sdm_pageview, root_logger)
...
def calculate_device_features(df: DataFrame):
    return (df
            ...
            .withColumn('DEVICE_BRAND_NAME', F.when((F.col('USER_AGENT').contains('iphone')) | (F.col('USER_AGENT').contains('macintosh')) | (F.col('USER_AGENT').contains('ipad')), 'apple')
                                .when(F.col('USER_AGENT').contains('nokia'), 'nokia')
                                .when((F.col('USER_AGENT').contains('huawei')) | (F.col('USER_AGENT').contains("MARX-L")), 'huawei')
                                .when((F.col('USER_AGENT').contains('samsung')) | (F.col('USER_AGENT').contains('sm-t')), 'samsung')
                                .when(F.col('USER_AGENT').contains('redmi') | (F.col('USER_AGENT').contains("mi")), 'xiaomi')
            ...
            .withColumn('DEVICE_MARKETING_NAME', F.when(F.col('USER_AGENT').contains('iphone'), 'iphone')
                                .when(F.col('USER_AGENT').contains('macintosh'), 'mac')
                                .when(F.col('USER_AGENT').contains('ipad'), 'ipad')
                                .when((F.col('USER_AGENT').contains('lenovo tab')) | (F.col('USER_AGENT').contains('lenovo tb')), 'lnv_tab')
                                ...
                                .when(F.col('USER_AGENT').contains('pixel'), F.regexp_extract(F.col("USER_AGENT"),"(pixel)(\\s)(\\d+)",0))
                                .when(F.col('USER_AGENT').contains('redmi'), F.regexp_extract(F.col("USER_AGENT"),"(redmi)(\\s\\w+)*{2}",0))
                                .when(F.col('USER_AGENT').contains(' mi '), F.regexp_extract(F.col("USER_AGENT"),"(mi)(\\s\\w+)*{2}",0)))
            .withColumn('OS_NAME', F.when(F.col('USER_AGENT').contains('iphone'), 'ios')
                                   ...
                                   .when((F.col('USER_AGENT').contains('ubuntu')) | (F.col('USER_AGENT').contains("CrOS")) | (F.col('USER_AGENT').contains("x86_64")), 'linux'))
            .withColumn('OS_VERSION', F.when(F.col('USER_AGENT').contains('android'), F.regexp_extract(F.col("USER_AGENT"),"android (\\d+)",1))
                                    .when(F.col('USER_AGENT').contains('iphone'), F.regexp_extract(F.col('USER_AGENT'), '(\\d+)(_)(\\d)', 0))
                                    .when(F.col('USER_AGENT').contains('windows'), F.regexp_extract(F.col("USER_AGENT"),"nt (\\d+)",1)))
            .withColumn("DEVICE_CATEGORY", F.when(F.col('OS_NAME').isin(desktop_os), "desktop")
                                     .when((F.col('OS_NAME') == 'ios') | ((F.col('OS_NAME') == 'android') & ((~F.col('DEVICE_MARKETING_NAME').isin(tablet_names)) | (F.col('DEVICE_MARKETING_NAME').isNull()) | (F.col('DEVICE_BRAND_NAME').isNull()))), "mobile")
                                     .when(F.col('DEVICE_MARKETING_NAME').isin(tablet_names), "tablet")
                                     .when(F.col('DEVICE_MARKETING_NAME')== 'smart_tv', "TV"))
            .withColumn('BROWSER_NAME', F.when(F.col('USER_AGENT').contains('firefox'), 'mozilla')
                                     .when((F.col('USER_AGENT').contains('mac')) & (~F.col('USER_AGENT').contains('chrome')) &  (~F.col('USER_AGENT').contains('firefox')) & (~F.col('USER_AGENT').contains('edg')) & (~F.col('USER_AGENT').contains('sznprohlizec')) & (~F.col('USER_AGENT').contains('opr')) & (~F.col('USER_AGENT').contains('crios')), 'safari')
                                     ...
                                     .otherwise('chrome'))
            .withColumn('DEVICE_MARKETING_NAME', F.regexp_replace('DEVICE_MARKETING_NAME', 'build', ''))
         )
...
def extract_url_formats(df: DataFrame):
    #NORMALIZED URL
    return df_url_normalization(df.withColumn("URL_NORMALIZED", F.col("URL")), "URL_NORMALIZED")
...
def order_into_sessions(df: DataFrame):
    ...
    df = (df
          .withColumn("event_last", F.lag("TIMESTAMP", 1).over(window_spec)).withColumn("event_difference", F.unix_timestamp("TIMESTAMP") - F.unix_timestamp("event_last"))
          .withColumn("diff_in_minutes", F.col("event_difference")/60)
          .withColumn("splits", (F.col("diff_in_minutes") >= 30).cast("integer")).fillna(value=0, subset=["splits", "event_difference"])
          .withColumn("TIME_GROUPING", F.sum("splits").over(window_spec)).fillna(value=0, subset=["diff_in_minutes"])
         )

    df = df.repartition(F.col("DEVICE"), F.col("TIME_GROUPING"))
    window_spec2 = Window.partitionBy("DEVICE","TIME_GROUPING").orderBy("TIMESTAMP")
    df = (df
          .withColumn("event_last", F.lag("TIMESTAMP", 1).over(window_spec2)).withColumn("event_difference", F.unix_timestamp("TIMESTAMP") - F.unix_timestamp("event_last"))
          .withColumn("diff_in_minutes", F.col("event_difference")/60).fillna(value=0, subset=["diff_in_minutes"])
          .withColumn("session_duration", F.sum("diff_in_minutes").over(window_spec2))
          .withColumn("REFERER", F.first("REFERER").over(window_spec2))
         )

    return (df.withColumn("zipped_events", F.array("URL", "TIMESTAMP", "URL_NORMALIZED", "OWNER_ID", "OWNER_NAME", "SEARCH_ENGINE", "FLAG_ADVERTISER", "FLAG_PUBLISHER"))
           )
...
def group_into_sessions(df: DataFrame):
    grouped_sessions = (df
                        .groupBy("DEVICE",
                                 ...
                                 "OS_VERSION",
                                 "BROWSER_NAME")
                        .agg(F.max("session_duration").alias("TOTAL_TIME_IN_SESSION"),
    ...
    grouped_sessions = grouped_sessions.select("*", F.col("NUMBER_OF_EVENTS").alias("NUMBER_OF_PAGE_VIEWS"))

    grouped_sessions = df_url_normalization(grouped_sessions.withColumn("REFERER_NORMALIZED", F.col("REFERER")), "REFERER_NORMALIZED")
...
def add_empty_cols_and_zipped_device(df: DataFrame):
    return (df
            .withColumnRenamed("REFERER_NORMALIZED", "REFERRAL_PATH_NORMALIZED")
            ...
            .withColumn("ZIPPED_DEVICE_DETAILS", F.array("DEVICE_CATEGORY", "DEVICE_BRAND_NAME", "DEVICE_MARKETING_NAME", "OS_NAME", "OS_VERSION", "BROWSER_NAME"))
           )
    
df_add_empty_cols_and_zipped_device = add_empty_cols_and_zipped_device(df_group_into_sessions)
...
def create_foreign_dimension_keys(df: DataFrame):
    ...
    return (
        ...
        .withColumn("DEVICE_ID", sdm_hash("DEVICE_CATEGORY",
                                        "DEVICE_BRAND_NAME",
                                    "DEVICE_MARKETING_NAME"))
        .withColumn("OS_ID", sdm_hash("OS_NAME",
                                     "OS_VERSION"))
    )

df_create_foreign_dimension_keys = create_foreign_dimension_keys(df_add_empty_cols_and_zipped_device)
...
write_dataframe_to_table(df_save_preprocessed_table, 
