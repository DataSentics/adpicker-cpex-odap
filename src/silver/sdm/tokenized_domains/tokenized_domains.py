...
from process_loaded_interests import process_loaded_interests
...
dbutils.widgets.dropdown("tokens_version", "cleaned_unique", ["cleaned_unique", "stemmed_unique"], "Tokens version")
...
def load_sdm_pageview(df: DataFrame, end_date: str, n_days: str):
    ...
    return (df
            .filter((F.col("page_screen_view_date") >= start_date) & (F.col("page_screen_view_date") < end_date))
            .select(
              "user_id",
              "full_url",
              "URL_NORMALIZED",
...
def load_sdm_url(df: DataFrame, tokens_version, use_bigrams, logger: Logger):
    ...
    if tokens_version == "cleaned_unique":
        ...
        logger.warning(f"{tokens_version} is not supported. Tokens are sourced from column: {tokens_col}")
...
df_sdm_url = load_sdm_url(df_silver_sdm_url, widget_tokens_version, widget_use_bigrams, root_logger) 
...
def create_tokenized_domains(df_url_tokenized: DataFrame, subinterests):
    ...
    return (
        output.join(F.broadcast(vocabulary), output.TOKEN == vocabulary.value)
        .drop("VALUE")
    )
