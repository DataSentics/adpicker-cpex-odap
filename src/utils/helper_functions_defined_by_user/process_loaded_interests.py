...
def _cleaned_stemmed_keywords(
    df: DataFrame, input_col: str, output_col: str, keyword_variant: str, bigrams=False
):
    ...
    if bigrams:
        ...
        df = df.withColumn(
            "keyword_cleaned", F.array(F.concat_ws(" ", F.col("keyword_cleaned")))
        ).withColumn(
            "keyword_stemmed", F.array(F.concat_ws(" ", F.col("keyword_stemmed")))
        )

    df = df.groupBy("subinterest", "general_interest").agg(
        F.array_distinct(F.flatten(F.collect_list("keyword_cleaned"))).alias(
            "keywords_cleaned"
        ),
        F.array_distinct(F.flatten(F.collect_list("keyword_stemmed"))).alias(
            "keywords_stemmed"
        ),
    )
    ...
    return df.select(
        "subinterest", "general_interest", F.col(keywords_col).alias(output_col)
    )


def process_loaded_interests(
    df: DataFrame,
    general_interests=False,
    use_bigrams=False,
    keyword_variant="cleaned",
    name_prefix="ad_interest_affinity_",
):
    ...
    df_kws = _cleaned_stemmed_keywords(
        df=df,
        input_col="keywords",
        output_col="keywords",
    ...
    if general_interests:
        df_kws = (
            df_kws.groupBy("general_interest")
            .agg(
                F.array_distinct(F.flatten(F.collect_list("keywords"))).alias(
                    "keywords"
                ),
            )
    ...
    if use_bigrams:
        df_kws_bi = _cleaned_stemmed_keywords(
            df=df,
            input_col="keywords_bigrams",
            output_col="keywords_bigrams",
            keyword_variant=keyword_variant,
            bigrams=True,
        )
        ...
        if general_interests:
            df_kws_bi = (
                df_kws_bi.groupBy("general_interest")
                .agg(
                    F.array_distinct(
                        F.flatten(F.collect_list("keywords_bigrams"))
                    ).alias("keywords_bigrams"),
                )
                .withColumn("subinterest", F.col("general_interest"))
            )

        df_kws = (
            df_kws.join(df_kws_bi, on=["subinterest", "general_interest"])
            .withColumn(
                "keywords", F.concat(F.col("keywords"), F.col("keywords_bigrams"))
            )
    ...
    interests_tuple = {
        name_prefix
        + row["subinterest"].lower(): Interest(
