...
def calculate_count_coefficient(col: Column) -> Column:
    ...
    Constants are designed in such a way so the function rises more quickly at smaller numbers,
    the rise slows down as number of sites rises and maxes out at 30.

    :param col: pyspark column containing number of sites visited
    :return: pyspark column with scaling count coefficients
    """

    return F.when(col <= 30, (col - 3 / 4) ** (1 / 4)).otherwise(
        (30 - 3 / 4) ** (1 / 4)
    )
