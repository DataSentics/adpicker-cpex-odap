...
def display_list_as_dataframe(
    l: list, name: str = "list", logger: Logger = None
) -> pd.DataFrame:
    """
...
def add_binary_flag_widget(widgets, name: str, label: str = None):
    ...
    :return: None
    """
    kwargs_default = {"choices": ["True", "False"], "default_value": "False"}
...
def get_top_n_sorted(df: DataFrame, column_name: str, n: int) -> DataFrame:
    ...
    """
    return df.sort(F.desc(column_name)).limit(n)
...
def flatten_string_iterable(src: Iterable[str], new_collection: Callable = set):
    ...
    """
    return new_collection([str_token for text in src for str_token in text.split()])
...
def format_fraction(num: float, denom: float) -> str:
    ...
    """
    return (
        f"{num:,} / {denom:,} ~ {(float('inf') if denom == 0 else (num / denom)):.2%}"
    )


def replace_categorical_levels(
    df: DataFrame, column_name: str, column_cats: list, replace_with: str = "None"
):
    """
    ...
    return df.withColumn(
        column_name,
        F.when(F.col(column_name).isin(column_cats), replace_with).otherwise(
            F.col(column_name)
        ),
    )
