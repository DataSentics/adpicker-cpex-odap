...
# MAGIC
# MAGIC This notebook serves to calculate URL scores for income models.
# MAGIC  The most common URLs are given a score based on marketing surveys (stored in azure storage).
# MAGIC  These scores are then joined to users' pageview and these values are averaged for each user, scaled by total number of sites visited.
# MAGIC  We transform and standardize those values to produce final URL scores for each income bracket.
# MAGIC
...
def load_url_scores():
    df = spark.read.format("delta").load(
        get_value_from_yaml("paths", "income_url_coeffs")
    )
