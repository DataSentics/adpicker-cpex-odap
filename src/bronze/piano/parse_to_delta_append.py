...
def create_bronze_cpex_piano(series_length):
    if series_length == 'long':
        if not delta_table_exists(get_value_from_yaml("paths", "piano_table_paths", "cpex_table_piano")):
...
def get_relevant_folders(base_path, destination_max_date, series_length, n_hours_short, n_days_long):
    paths = dbutils.fs.ls(base_path)
    max_date = max([datetime.strptime(p.name.replace('day=', '').replace('/', ''), '%Y-%m-%d').date() for p in paths])
    ...
    if series_length == 'long':
        ...
        return [p.path for p in paths
                if datetime.strptime(p.name.replace('day=', '').replace('/', ''), '%Y-%m-%d').date() >= date_limit]
...
base_path = "/mnt/piano"
relevant_folders_list = get_relevant_folders(base_path,
                                            df_max_upload_date,
...
def get_jsons(relevant_folders, logger: Logger):
    ...
    for folder in relevant_folders:
        ...
        for elem in files_in_folder:
            ...
            name = re.search('.*\d{4}-\d{2}-\d{2}/(.*?_*[0-9]+)\.json', elem_path).group(1)
            ...
            owner_name = re.search('.*\d{4}-\d{2}-\d{2}/(.*?)_*[0-9]+\.json', elem_path).group(1)
            if not owner_name:
                owner_name = "unknown"
...
def load_raw_delta(relevant_folders, series_length, destination_max_date, n_hours_short, logger: Logger):
    ...
    df = (df
    .withColumnRenamed("url", "rp_pageurl")
    ...
    .withColumn("AGE", get_custom_parameters(F.col("customParameters"), param_of_interest="ub"))
    .withColumn("GENDER", get_custom_parameters(F.col("customParameters"), param_of_interest="us")) 
    )
    if series_length == 'short':
        # short series only n hours of history 
        min_date = df.agg(F.max('EVENT_TIME') - F.expr(f'INTERVAL {n_hours_short} HOURS')).collect()[0][0]
...
df_raw_data = load_raw_delta(relevant_folders_list, 
                             widget_series_length, 
                             df_max_upload_date,
                             get_value_from_yaml("tables_options", "series_length", "n_hours_short"), 
                             root_logger)
