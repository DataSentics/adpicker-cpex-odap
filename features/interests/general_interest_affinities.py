...
# for the seak of printing
indented_metadata = json.dumps(metadata, indent=4)
print(indented_metadata)


# COMMAND ----------

# MAGIC %md Add interests to domains
...
display(df_tokenized_domains_interests)
...
display(df_joined_interests_with_stats)

# COMMAND ----------

def features_digi_interests(joined_interests_with_stats: DataFrame, interest_names: list, entity):
    interest_affinities = indexes_calculator(
        joined_interests_with_stats,
        interest_names,
        windows_number=1,
        level_of_distinction=[entity.id_column, entity.time_column],
    )
