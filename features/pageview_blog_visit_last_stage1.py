...
def calculate_days_since_blog_visit(df):
    ...
    df_group = df_days_column.groupby("user_id").agg(
        F.min("days_since_blog_view").alias(
            f"web_analytics_blog_last_visit_date_{time_window_str}"
        )
    )
    return df_group
...
metadata = {
    ...
    "features": {
        f"web_analytics_blog_days_since_last_visit_{time_window_str}": {
            "description": f"Number of days since the last visit of the blog on the web page in last {time_window_str}."
        },
       "fillna_with": None,
    },
}
