...
def process_age_categories(df: DataFrame):
    return (
        ...
        .withColumn(
            "GENDER",
            F.when(F.col("GENDER") == "1", "0").when(F.col("GENDER") == "2", "1"),
        )
    )
...
def aggregate_targets(df: DataFrame, df_fs: DataFrame):
    df = (
        ...
        .agg(
            *[
                F.array_distinct(F.collect_set(x)).alias(x)
                for x in ["LABEL_AGE", "GENDER"]
            ],
        )
        ...
        .withColumn(
            ...
            F.when(F.size("LABEL_AGE") == 1, F.col("LABEL_AGE").getItem(0)).otherwise(
                "unknown"
            ),
        )
        .withColumn(
            "sociodemo_targets_gender",
            F.when(F.size("GENDER") == 1, F.col("GENDER").getItem(0)).otherwise(
                "unknown"
            ),
        )
    )
    # join to feature store records
    return df_fs.join(df, how="left", on="user_id").select(
        "user_id", "timestamp", "sociodemo_targets_age", "sociodemo_targets_gender"
    )
