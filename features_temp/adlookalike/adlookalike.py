...
feature_store = spark.read.format("delta").load(get_value_from_yaml("paths", "feature_store_paths", "user_entity_fs"))
...
metadata = get_prediction_cols(df_load_lookalikes_to_score, "user", "lookalike_features")
display(metadata)
...
df_final = features_lals_predictions(df_lookalikes_scoring, list(metadata['features'].keys()))
